...
- **正解 = 1シャンテンに進む打牌のうち、テンパイへの受け入れ枚数が最大の打牌**。受け入れ枚数が同数の場合は**変化（改良ツモ）の枚数が多い方だけが正解**。変化まで同数の手や、孤立字牌が2種類以上あって優劣を説明できない手は出題せず、正解を必ず1種類に絞る。
- 回答後、打牌候補ごとの受け入れ枚数・受け入れ牌・変化の一覧を表示する。
- 初心者が間違えやすい形を「罠型」として判定する。孤立字牌（役牌期待で残しやすい）、浮き牌の質（3〜7が最も広い）、唯一の対子（崩すとヘッドレスになる）、二度受け（2つの搭子が同じ牌を待つ）の4種。半数の問題で罠型を優先的に探し、見つからない場合は通常問題へフォールバックするため、**実際に罠型が付く割合は約30%**（40問での実測）。回答後は該当する考え方を解説カードに表示する。
